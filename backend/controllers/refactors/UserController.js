...
const deleteUsuari = async (req, res, next) => {
  try {
    ...
    const metasCount = await prisma.meta.count({
      where: { author_id: id },
    });
    ...
    if (metasCount > 0 || groupsCount > 0) {
      const error = new Error(
        "No es pot eliminar l'usuari: té metes o grups associats",
      );
      error.statusCode = 409;
      throw error;
    }

    await prisma.groupUser.deleteMany({ where: { user_id: id } });
    ...
    await prisma.comment.deleteMany({ where: { user_id: id } });
    await prisma.indexedMeta.deleteMany({ where: { user_id: id } });
